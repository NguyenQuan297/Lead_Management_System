...
- Set strong values for:
  - `DEFAULT_ADMIN_EMAIL` and `DEFAULT_ADMIN_PASSWORD` – used to create the first admin if no users exist
...
- On first run, if no users exist, a default admin is created using **environment variables** (no hardcoded credentials in code):
  - `DEFAULT_ADMIN_EMAIL` – admin login email
  - `DEFAULT_ADMIN_PASSWORD` – admin login password
- Set these in `.env` before first run. Use a strong password in production.
